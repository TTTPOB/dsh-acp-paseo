...
1. `initialize` → 协议版本一致
2. `session/new` → **目录非空 + currentModelId + modes[execute,plan] + thought_level 选项**
3. 命令广播 → 含 compact/plan/goal（`/export` 被过滤）
4. `set_mode plan` → `unstable_setSessionModel` → `set_config_option thought_level=high` 逐项生效
