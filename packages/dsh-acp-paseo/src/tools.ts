 ...
 */
export function toolKindForName(name: string): AcpToolKind | undefined {
  if (name === 'subagent' || name === 'subagent_fork') return undefined
 ...
 * cannot be read.
 */
export function toolTitleFor(name: string, argumentsJson: string): string {
 ...
 */
export function renderToolResultText(blocks: readonly { readonly type: string; readonly text?: string }[]): string {
  const text = blocks
